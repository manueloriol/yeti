...
Distributing YETI over the cloud requires uploading the files containing the options for launching YETI in standalone mode, and 
YETI itself (bundled with classes to be tested). Setting up the cluster and uploading all the required files to the 
MapReduce Master would then need very little time (under 5 minutes), if the user account has already been set up. Setting up a cluster  
for the first time might take a little extra time(up to 10 minutes), but subsequent testing sessions would require only a 
few minutes to launch a testing session.
