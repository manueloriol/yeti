...
Other semantics are however possible. For example Random+~\cite{CMOP:08:FFMTRTUR} either picks selects values in the existing pool of object, generate new ones on the fly, or take interesting values. This would then mean a different semantics for $selectVariableFromType$.

As previously shown, several specializations of the model are possible. The next chapter will present the both the implementation of the core of YETI and the implemenation of the Java binding.
