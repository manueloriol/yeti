...
YETI is an application coded in Java, that allows to test programs at random in a fully automated manner.
...
YETI is a lightweight platform with around 5000 lines of code for the core, strategies and the Java binding.
 As explained in Section~\ref{sec:model}, the core infrastructure provides extendibility trough the specialization. 
...
\texttt{-testModules=java.lang.String} indicates that both \texttt{String} and
\texttt{StringBuilder} will both be tested.

While testing traces of faults found are reproduced in the terminal. For example:
...
\begin{center}
%\begin{sideways}
\includegraphics[width=13cm]{images/YetiComplete.png}
%\end{sideways}
\end{center}
...
In short each time a test is made, each paramater of the routine to test can either 
be void, be newly generated or a new variable. These sliders indicate which possibility 
...
create variables to use when making routine calls. A button is available to add modules 
...
The first one samples data for building graphs every .1 seconds, the second one updates the graphical user interfaces and waits .1 second between two updates. A special care has also been taken for not showing all samples in the graphs when not needed: we only show one point per pixel on the x-axis. In ten minutes testing sessions  of \texttt{java.lang.String} on a dual core MacBook Pro, the slowdown incurred by the GUI was measured to be $4.4\%$. 
...
path. Each of these classes then defines both a module and a type. None of the classes in the transitive closure of classes defines either a module or a type. This is mainly due to performance optimizations 
as this would automatically result in loading at least 30 classes in the system, some of which have a very 
negative impact on performances of the system (for example \texttt{java.lang.StringBuffer}).
Instead, test engineers might decide to load such classes -- or other helper classes -- by hand in the graphical user interface.

Subsequently, each method of each loaded class is added to its module, while any method (including 
...
\item[Random+] that adds the utilisation of ``interesting'' values. For the Java binding such values for integers contain \texttt{MAX\_INT}, \texttt{MIN\_INT}, all values between $-10$ and $10$ etc. It also defines the probability to use such interesting values.
...
While the first two strategies were described previously in literature~\cite{}, the last two are new as nobody thought of modifying the probabilities over time.
