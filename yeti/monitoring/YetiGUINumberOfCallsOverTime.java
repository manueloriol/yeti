 ...
 *
 */
public class YetiGUINumberOfCallsOverTime extends YetiMonitoringLogProcessor {
	 ...
	 */
	public YetiGUINumberOfCallsOverTime(YetiLogProcessor lp, int nMilliseconds) {
		this.nMilliseconds = nMilliseconds;
		this.lp=lp;
		JFrame f = new JFrame();
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		graph = new YetiGraph("Number of Calls, steps of "+nMilliseconds+" millisecond");
		f.add(graph);
		f.setSize(400,200);
	...
	public void appendToCurrentLog(String newLog) {
		...
		if (!called) {
			...
			called = true;
			// and add the value
			graph.addValue(firstInstant,YetiLog.numberOfCalls);
		} else {
			...
			if (lastInstantShown+1<currentSecond) {
				graph.addValue(currentSecond-firstInstant,YetiLog.numberOfCalls);				
